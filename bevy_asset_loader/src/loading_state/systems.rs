...
pub(crate) fn start_loading_collection<S: States, Assets: AssetCollection>(
    ...
    info!("starting to load collection");
...
pub fn apply_internal_state_transition<S: States>(world: &mut World) {
        ...
        .is_some()
    {
        ...
        if world
            .resource::<Schedules>()
            .contains(&OnEnterInternalLoadingState(state.clone(), exited_state))
        {
            world.run_schedule(OnEnterInternalLoadingState(state, exited_state));
        }
    }
}
